...
/* ---- 匯出／分享文件產生器（英文輸出：這些文件是給美國大學／老師看的） ---- */
function usEsc(s) { return String(s == null ? '' : s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c])); }
function usActLine(d) {
  const meta = [d.type, (d.grades || []).length ? 'Grades ' + (d.grades || []).join(', ') : '', d.timing,
    d.hours ? d.hours + ' hrs/wk' : '', d.weeks ? d.weeks + ' wks/yr' : ''].filter(Boolean).join(' · ');
  return { head: (d.position || '(role)') + (d.org ? ' — ' + d.org : ''), meta: meta, desc: d.description || '', refl: d.reflection || '' };
}
function usCommonAppText(acts) {
  if (!acts.length) return 'No activities yet. Add a few first, then copy your Common App list here.';
  const head = 'COMMON APP — ACTIVITIES LIST\n(Ordered as entered — put your most important activity first.)\n';
  const body = acts.map((a, i) => {
    const d = a.data || {};
    return [
      (i + 1) + '. ' + (d.type || 'Activity'),
      '   Position/Leadership: ' + (d.position || ''),
      '   Organization: ' + (d.org || ''),
      '   Grades: ' + ((d.grades || []).join(', ') || '—') + '   Timing: ' + (d.timing || '—'),
      '   Hours/week: ' + (d.hours || '—') + '   Weeks/year: ' + (d.weeks || '—'),
      '   Description (' + (d.description || '').length + '/150): ' + (d.description || ''),
    ].join('\n');
  }).join('\n\n');
  return head + '\n' + body;
}
function usDocShell(title, inner) {
  return '<!doctype html><html><head><meta charset="utf-8"><title>' + usEsc(title) + '</title><style>'
    + '@page{margin:2cm}*{box-sizing:border-box}body{font-family:Georgia,\'Times New Roman\',serif;color:#1a1a1a;line-height:1.4;max-width:720px;margin:0 auto;padding:18px}'
    + 'h1{font-size:20pt;margin:0 0 2px}h2{font-size:12pt;text-transform:uppercase;letter-spacing:.06em;border-bottom:1.5px solid #1a1a1a;padding-bottom:3px;margin:16px 0 8px}'
    + '.muted{color:#555;font-size:10pt}.row{margin:8px 0}.hd{font-weight:bold}.meta{color:#555;font-size:9.5pt}.desc{font-size:10.5pt;margin-top:2px}'
    + '.refl{font-size:10pt;color:#333;margin-top:3px;font-style:italic}ul{margin:4px 0;padding-left:18px}li{margin:3px 0}'
    + '.note{font-size:9pt;color:#777;margin-top:22px;border-top:1px solid #ccc;padding-top:8px}'
    + '@media print{.noprint{display:none}}.noprint{position:fixed;top:10px;right:10px}'
    + 'button{font-family:sans-serif;background:#16233b;color:#fff;border:0;border-radius:8px;padding:9px 16px;font-weight:700;cursor:pointer}'
    + '</style></head><body>'
    + '<div class="noprint"><button onclick="window.print()">Save as PDF / Print</button></div>'
    + inner + '</body></html>';
}
function usHeaderBlock(student) {
  const name = usEsc(student.name || 'Student');
  const bits = [student.school_name ? usEsc(student.school_name) : '', student.grad_year ? 'Class of ' + usEsc(student.grad_year) : '', student.email ? usEsc(student.email) : ''].filter(Boolean).join(' · ');
  return '<h1>' + name + '</h1><div class="muted">' + bits + '</div>';
}
function usResumeHTML(student, recs) {
  const acts = recs.filter((r) => r.kind === 'activity');
  const hons = recs.filter((r) => r.kind === 'honor');
  let h = usHeaderBlock(student);
  h += '<h2>Education</h2><div class="row">' + usEsc(student.school_name || 'High School')
    + (student.grad_year ? ' — expected graduation ' + usEsc(student.grad_year) : '')
    + (student.rigor_track ? '<div class="meta">Course rigor: ' + usEsc(student.rigor_track) + '</div>' : '') + '</div>';
  if (acts.length) {
    h += '<h2>Activities &amp; Leadership</h2>';
    acts.forEach((a) => { const L = usActLine(a.data || {}); h += '<div class="row"><span class="hd">' + usEsc(L.head) + '</span>' + (L.meta ? '<div class="meta">' + usEsc(L.meta) + '</div>' : '') + (L.desc ? '<div class="desc">' + usEsc(L.desc) + '</div>' : '') + '</div>'; });
  }
  if (hons.length) {
    h += '<h2>Honors &amp; Awards</h2><ul>';
    hons.forEach((x) => { const d = x.data || {}; h += '<li><span class="hd">' + usEsc(d.title || '') + '</span> <span class="meta">— ' + usEsc(d.level || '') + (d.grade ? ', Grade ' + usEsc(d.grade) : '') + '</span></li>'; });
    h += '</ul>';
  }
  h += '<div class="note">Generated by TIPS from four years of the student’s own captured work. TIPS guides reflection only — it does not write or submit applications.</div>';
  return usDocShell((student.name || 'Student') + ' — Resume', h);
}
function usBragHTML(student, recs) {
  const acts = recs.filter((r) => r.kind === 'activity');
  const hons = recs.filter((r) => r.kind === 'honor');
  let h = usHeaderBlock(student);
  h += '<div class="muted" style="margin-top:6px">Brag sheet — background for the teacher or counselor writing my recommendation. In my own words, with reflections.</div>';
  if (student.focus_anchor) h += '<h2>Where I’m headed</h2><div class="row">Intended focus: ' + usEsc(student.focus_anchor) + '</div>';
  if (acts.length) {
    h += '<h2>What I’ve done — and what it meant to me</h2>';
    acts.forEach((a) => {
      const d = a.data || {}; const L = usActLine(d);
      h += '<div class="row"><span class="hd">' + usEsc(L.head) + '</span>' + (L.meta ? '<div class="meta">' + usEsc(L.meta) + '</div>' : '') + (L.desc ? '<div class="desc">' + usEsc(L.desc) + '</div>' : '') + (L.refl ? '<div class="refl">“' + usEsc(L.refl) + '”</div>' : '') + '</div>';
    });
  }
  if (hons.length) {
    h += '<h2>Honors &amp; Awards</h2><ul>';
    hons.forEach((x) => { const d = x.data || {}; h += '<li><span class="hd">' + usEsc(d.title || '') + '</span> <span class="meta">— ' + usEsc(d.level || '') + (d.grade ? ', Grade ' + usEsc(d.grade) : '') + '</span>' + (d.reflection ? '<div class="refl">“' + usEsc(d.reflection) + '”</div>' : '') + '</li>'; });
    h += '</ul>';
  }
  h += '<div class="note">Shared by the student to help you write a specific, personal letter. Prepared with TIPS — reflections are the student’s own.</div>';
  return usDocShell((student.name || 'Student') + ' — Brag Sheet', h);
}
function usParentHTML(student, recs) {
  const acts = recs.filter((r) => r.kind === 'activity');
  const hons = recs.filter((r) => r.kind === 'honor');
  const esss = recs.filter((r) => r.kind === 'essay');
  let h = '<h1>' + usEsc(student.name || 'Your student') + '’s progress</h1>';
  h += '<div class="muted">A quick, read-only summary of what has been captured so far.</div>';
  h += '<h2>At a glance</h2><ul><li>' + acts.length + ' activities logged</li><li>' + hons.length + ' honors &amp; awards</li><li>' + esss.length + ' essay draft(s) started</li></ul>';
  if (acts.length) { h += '<h2>Activities captured</h2><ul>'; acts.forEach((a) => { const L = usActLine(a.data || {}); h += '<li><span class="hd">' + usEsc(L.head) + '</span>' + (L.meta ? ' <span class="meta">— ' + usEsc(L.meta) + '</span>' : '') + '</li>'; }); h += '</ul>'; }
  if (hons.length) { h += '<h2>Honors</h2><ul>'; hons.forEach((x) => { h += '<li>' + usEsc((x.data || {}).title || '') + '</li>'; }); h += '</ul>'; }
  h += '<div class="note">The goal is steady capture over four years — a few entries each semester beats a last-minute scramble. TIPS guides reflection only; it never writes essays or guarantees admission.</div>';
  return usDocShell((student.name || 'Student') + ' — Parent Summary', h);
}
function usOpenPrint(html) {
...
function USOverview({ student, lang }) {
  ...
  const [narrOut, setNarrOut] = useState('');
  ...
  async function askNarrative() {
    setNarrBusy(true); setNarrOut('');
    try { const d = await api('usNarrative', {}); setNarrOut(d.text); }
    catch (e) { setNarrOut((t('教練暫時無法回應：', 'Coach unavailable: ', '教练暂时无法回应：')) + e.message); }
    setNarrBusy(false);
  }
  async function uploadEvidence(file) {
    ...
    try {
      ...
      if (isImg && file.size > 4 * 1048576) { ff = await compressImage(file, 4); if (!ff) throw new Error(t('圖片太大，請縮小再傳', 'Image too large — resize and try again', '图片太大，请缩小再传')); }
      if (ff.size > 10 * 1048576) throw new Error(t('檔案超過 10MB', 'File exceeds 10MB', '文件超过 10MB'));
      const base64 = await fileToBase64(ff);
      const up = await api('uploadFile', { base64, filename: ff.name, mimeType: ff.type });
      setF(Object.assign({}, f, { evidence_url: up.file_url, evidence_name: ff.name }));
    } catch (e) { setErr(e.message); }
    setUpBusy(false);
  }
  function copyCommonApp() {
    const txt = usCommonAppText(byKind('activity'));
    try { navigator.clipboard.writeText(txt); setCopied('commonapp'); setTimeout(() => setCopied(''), 2500); }
    catch (e) { usOpenPrint(usDocShell('Common App Activities List', '<pre style="white-space:pre-wrap;font-family:Georgia,serif;font-size:10.5pt">' + usEsc(txt) + '</pre>')); }
  }
  ...
  const totalItems = acts.length + hons.length + esss.length;
  ...
  const REFL_STYLE = Object.assign({}, US_FLD, { minHeight: 52 });
  ...
  return (
    <div style={{ padding: '10px 4px 90px' }}>
      ...
      {/* ===== Narrative coach ===== */}
      <div style={{ background: '#16233b', color: '#f7f7f4', borderRadius: 14, padding: '14px 16px', margin: '12px 0 6px' }}>
        <div style={{ fontWeight: 800, fontSize: '1rem' }}>{t('✦ 找出你的主軸', '✦ Find your through-line', '✦ 找出你的主轴')}</div>
        <div style={{ fontSize: '.85rem', opacity: .85, margin: '4px 0 10px' }}>{t('教練會讀你整個四年的紀錄，找出貫穿的主題與亮點，並給 essay 方向——只給問題與方向，不代寫。', 'The coach reads your whole four-year record, finds the theme that runs through it and your spike, and suggests essay directions — questions and directions only, never a draft.', '教练会读你整个四年的记录，找出贯穿的主题与亮点，并给 essay 方向——只给问题与方向，不代写。')}</div>
        <button style={{ background: '#ffde59', color: '#16233b', border: 'none', borderRadius: 9, padding: '9px 16px', fontWeight: 800, cursor: 'pointer' }} disabled={narrBusy || totalItems === 0} onClick={askNarrative}>
          {narrBusy ? t('教練思考中…', 'Coach is thinking…', '教练思考中…') : (totalItems === 0 ? t('先加幾筆紀錄再試', 'Add a few entries first', '先加几笔记录再试') : t('讀我的紀錄、找主軸', 'Read my record & find the thread', '读我的记录、找主轴'))}
        </button>
        {narrOut && <div style={{ background: '#fff', color: '#1c2433', border: '1.5px solid #ffde59', borderRadius: 10, padding: '12px 14px', marginTop: 10, fontSize: '.9rem', whiteSpace: 'pre-wrap' }}>{narrOut}</div>}
      </div>
      ...
      {acts.map((a) => (
        <div key={a.id} style={US_CARD}>
          ...
          {a.data.reflection && <div style={{ fontSize: '.84rem', color: '#2a3a5c', marginTop: 4, fontStyle: 'italic', borderLeft: '2px solid #ffde59', paddingLeft: 8 }}>“{a.data.reflection}”</div>}
          {a.data.evidence_url && <div style={{ marginTop: 4 }}><a href={a.data.evidence_url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '.8rem', fontWeight: 700 }}>📎 {t('證據附件', 'Evidence', '证据附件')}{a.data.evidence_name ? '：' + a.data.evidence_name : ''}</a></div>}
        </div>
      ))}
      {open === 'activity' && (
        <div style={Object.assign({}, US_CARD, { border: '2px solid #16233b' })}>
          ...
          {/* --- deepen: 60-second reflection + evidence --- */}
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px dashed #d9d9d2' }}>
            <div style={{ fontWeight: 700, fontSize: '.86rem' }}>{t('⏱ 60 秒反思（只有你看得到，日後寫 essay 的金礦）', '⏱ 60-second reflection (private — your essay gold later)', '⏱ 60 秒反思（只有你看得到，日后写 essay 的金矿）')}</div>
            <textarea style={REFL_STYLE} placeholder={t('你做了什麼？學到什麼／改變了什麼？卡在哪、怎麼解決的？', 'What did you do? What did you learn or change? Where did you get stuck, and how did you handle it?', '你做了什么？学到什么／改变了什么？卡在哪、怎么解决的？')} value={f.reflection || ''} onChange={(e) => set('reflection', e.target.value)} />
            <div style={{ marginTop: 6 }}>
              <span style={US_MUTED}>{t('證據附件（選填）：照片／PDF／作品', 'Evidence (optional): photo / PDF / work', '证据附件（选填）：照片／PDF／作品')}</span>
              <input type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'block', marginTop: 4, fontSize: '.85rem' }} onChange={(e) => uploadEvidence(e.target.files[0])} />
              {upBusy && <span style={{ fontSize: '.8rem', color: '#b06f00' }}>{t('上傳中…', 'Uploading…', '上传中…')}</span>}
              {f.evidence_url && !upBusy && <div style={{ fontSize: '.8rem', color: '#15703c', marginTop: 2 }}>✓ {t('已附上', 'Attached', '已附上')}{f.evidence_name ? '：' + f.evidence_name : ''} <button style={{ border: 'none', background: 'none', color: '#b3261e', cursor: 'pointer', fontSize: '.8rem' }} onClick={() => setF(Object.assign({}, f, { evidence_url: '', evidence_name: '' }))}>✕</button></div>}
            </div>
          </div>

          <div style={{ marginTop: 10 }}>
            <button style={US_BTN} disabled={busy || upBusy} onClick={save}>{busy ? t('儲存中…', 'Saving…', '保存中…') : t('儲存', 'Save', '保存')}</button>
      ...
      {hons.map((h) => (
        <div key={h.id} style={US_CARD}>
          ...
          {h.data.reflection && <div style={{ fontSize: '.84rem', color: '#2a3a5c', marginTop: 4, fontStyle: 'italic', borderLeft: '2px solid #ffde59', paddingLeft: 8 }}>“{h.data.reflection}”</div>}
        </div>
      ))}
      {open === 'honor' && (
        <div style={Object.assign({}, US_CARD, { border: '2px solid #16233b' })}>
          ...
          <textarea style={REFL_STYLE} placeholder={t('一句話：為什麼這個獎對你有意義？（寫 essay／推薦信會用到）', 'One line: why did this award matter to you? (useful for essays & recommendation letters)', '一句话：为什么这个奖对你有意义？（写 essay／推荐信会用到）')} value={f.reflection || ''} onChange={(e) => set('reflection', e.target.value)} />
      ...
      {/* ===== Export & Share ===== */}
      <div style={{ marginTop: 26, background: '#fff', border: '1.5px solid #16233b', borderRadius: 14, padding: '16px 18px' }}>
        <h3 style={{ margin: '0 0 4px' }}>{t('匯出與分享', 'Export & Share', '导出与分享')}</h3>
        <p style={{ ...US_MUTED, margin: '0 0 12px' }}>{t('把你四年的累積，一鍵變成申請當下真正要用的文件。', 'Turn four years of your own work into the documents you actually need at application time.', '把你四年的积累，一键变成申请当下真正要用的文件。')}</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(200px,1fr))', gap: 10 }}>
          <button style={{ ...US_BTN, textAlign: 'left' }} onClick={copyCommonApp}>{copied === 'commonapp' ? t('✓ 已複製', '✓ Copied', '✓ 已复制') : t('📋 複製 Common App 活動清單', '📋 Copy Common App activities list', '📋 复制 Common App 活动清单')}</button>
          <button style={{ ...US_BTN, textAlign: 'left' }} onClick={() => usOpenPrint(usResumeHTML(student, recs))}>{t('📄 一頁履歷（存成 PDF）', '📄 One-page résumé (save as PDF)', '📄 一页履历（存成 PDF）')}</button>
          <button style={{ ...US_BTN, textAlign: 'left' }} onClick={() => usOpenPrint(usBragHTML(student, recs))}>{t('📝 推薦信打包（給老師／顧問）', '📝 Brag sheet (for teachers/counselors)', '📝 推荐信打包（给老师／顾问）')}</button>
          <button style={{ ...US_BTN, textAlign: 'left' }} onClick={() => usOpenPrint(usParentHTML(student, recs))}>{t('👪 家長進度摘要', '👪 Parent progress summary', '👪 家长进度摘要')}</button>
        </div>
        <p style={{ fontSize: '.74rem', color: '#5a6378', marginTop: 10 }}>{t('文件皆以英文產出（給美國大學／老師）。清單依你輸入的順序排列——把最重要的放前面。推薦信打包會帶上你的反思，讓老師寫得更具體。', 'Documents are generated in English (for US colleges/teachers). The list follows your entry order — put your most important item first. The brag sheet includes your reflections so recommenders can write something specific.', '文件皆以英文产出（给美国大学／老师）。清单依你输入的顺序排列——把最重要的放前面。推荐信打包会带上你的反思，让老师写得更具体。')}</p>
      </div>
...
function InterviewCoach({ line, deptId, school, dept, onClose }) {
  ...
  const IV_FU_DEPTH = 2;                      // 每題追問來回上限（與後端一致）
      ...
      <div className="s2-card">
        ...
        {qText && (
          <>
            ...
            <p className="hint" style={{ marginTop: 8 }}>每個校系演練上限 10 次（含追問對答）；每題最多來回 {IV_FU_DEPTH} 輪追問。最後階段建議找老師或家人做真人模擬（計時＋錄音回聽）。</p>
          </>
        )}
      </div>
    </div>
